...
main() {
    ...
    if ! [[ "$namespace_count" =~ ^[0-9]+$ ]] || [ "$namespace_count" -lt 1 ] || [ "$namespace_count" -gt 100 ]; then
        log_error "Namespace count must be a number between 1 and 100"
        exit 1
    fi
...
case "${1:-deploy}" in
    ...
    "--help"|"-h"|"help")
        ...
        echo "                                      namespace_count: Number of namespaces to create (1-100, default: 4)"
        ...
        echo "  $0 deploy 200 100           # Deploy with maximum 200 IPs and 100 namespaces"
        ;;
    *)
